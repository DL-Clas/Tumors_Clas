...
if is_installed('swattention'):
    ...
    from net.attention_cuda import AggregatedAttention
else:
    print('swattention package not found, loading PyTorch native version of Aggregated Attention')
    from net.attention_native import AggregatedAttention
...
class TransNeXt(nn.Module):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
                 patch_size=16, in_chans=3, num_classes=10, embed_dims=[64, 128, 256, 512],
...
@register_model
def transnext_small(num_classes, pretrained=False, **kwargs):
    model = TransNeXt(num_classes = num_classes, window_size=[3, 3, 3, None],
                      patch_size=4, embed_dims=[72, 144, 288, 576], num_heads=[3, 6, 12, 24],
                      mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def transnext_base(num_classes, **kwargs):
    model = TransNeXt(num_classes = 5, window_size=[3, 3, 3, None],
...
@register_model
def transnext_micro_AAAA(num_classes, pretrained=False, **kwargs):
...
if __name__=='__main__':
    ...
    model = transnext_small(num_classes = 7)
