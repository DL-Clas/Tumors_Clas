...
def main(args):
    ...
    if os.path.exists(cache_file):
        ...
        print(f"⏭️ Skipping dataset loading and model inference, directly reading cache...\n")
        
        with open(cache_file, 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.strip().split('\t')
                # Now there are 5 parts: Fold_ID, True_Label, Pred_Label, Confidence, Image_Path
                if len(parts) == 5:
                    fold_idx, true_l, pred_l, conf, img_path = parts
        ...
        print(f"⚠️ No cache file with confidence detected, will load the model for inference.")
        ...
        for fold in range(1, args.k_folds + 1):
            ...
            with torch.no_grad():
                for images, labels, paths in test_loader:
                    ...
                    # ===============================================
                    # Add calculation: convert the output Logits into Softmax confidence probability.
                    # ===============================================
                    probs = torch.softmax(outputs, dim=1)
                    # Obtain the highest probability value (confs) and the corresponding category (preds).
                    confs, preds = torch.max(probs, dim=1)
                    
                    preds_cpu = preds.cpu().numpy()
                    confs_cpu = confs.cpu().numpy() # Extract Confidence
                    labels_cpu = labels.cpu().numpy()
                    
                    for i in range(len(paths)):
                        true_l = labels_cpu[i]
                        pred_l = preds_cpu[i]
                        conf = confs_cpu[i]         # Extract the confidence of the current sample
                        img_p = paths[i]
                        ...
                        # Write txt cache in real time, and add Confidence column.
                        # Format: Fold_ID \t True_Label \t Pred_Label \t Confidence \t Image_Path
                        f_cache.write(f"{fold}\t{true_l}\t{pred_l}\t{conf:.6f}\t{img_p}\n")
                        
        f_cache.close()
        print(f"✅ Inference completed, all results (with confidence) have been successfully cached to txt.\n")
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        for i, m in enumerate(fold_metrics):
            ...
            row.update(m)
            writer.writerow(row)
    print(f"\n✅ Fold-level raw evaluation results have been exported to: {csv_path}")
    ...
    print(f"\n✅ Objective heatmap evaluation completed, results saved to {model_results_dir} directory.")
    ...
    print(f"Total independent patients: {len(patient_results)}")
